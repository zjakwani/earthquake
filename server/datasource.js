...
class EarthquakeAPI extends RESTDataSource {
  ...
  async getEarthquakes({ lat, long, radius }) {
      const response = await this.get(`https://earthquake.usgs.gov/fdsnws/event/1/query?format=geojson&orderby=magnitude&starttime=2021-06-17&latitude=${lat}&longitude=${long}&maxradiuskm=${radius}`)
